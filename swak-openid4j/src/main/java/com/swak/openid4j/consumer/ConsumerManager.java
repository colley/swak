...
import com.swak.openid4j.association.DiffieHellmanSession;
import com.swak.openid4j.discovery.Discovery;
...
import com.swak.openid4j.server.IncrementalNonceGenerator;
import com.swak.openid4j.server.NonceGenerator;
import com.swak.openid4j.server.RealmVerifier;
import com.swak.openid4j.server.RealmVerifierFactory;
import com.swak.openid4j.util.HttpFetcher;
import com.swak.openid4j.util.HttpFetcherFactory;
import com.swak.openid4j.util.HttpRequestOptions;
import com.swak.openid4j.util.HttpResponse;
...
public class ConsumerManager {
    ...
    public void setMaxAssocAttempts(int maxAssocAttempts) {
        ...
        if (_maxAssocAttempts == 0) log.info("Associations disabled.");
    }
    ...
    private int call(String url, Message request, ParameterList response)
        ...
        try {
            ...
            if (DEBUG) log.debug("Performing HTTP POST on " + url);
            ...
            if (DEBUG) log.debug("Retrived response:\n" + postResponse);
        } catch (IOException e) {
            log.error("Error talking to " + url +
    ...
    public DiscoveryInformation associate(List discoveries) {
        ...
        if (discoveries.size() > 0) {
            ...
            log.error("Association attempt, but no discovery endpoints provided.");
            return null;
        }
    }
    ...
    private int associate(DiscoveryInformation discovered, int maxAttempts) {
        ...
        log.info("Trying to associate with " + opEndpoint +
        ...
        if (a != null &&
            ...
            log.info("Found an existing association: " + a.getHandle());
            return 0;
        }
        ...
        while (attemptsLeft > 0 && !reqStack.empty()) {
            try {
                ...
                if (alreadyTried.keySet().contains(assocReq.getType())) {
                    if (DEBUG) log.debug("Already tried.");
                    continue;
                }
                ...
                if (status == HttpStatus.SC_OK) // success response
                {
                    ...
                    if (respType.equals(assocReq.getType()) ||
                        ...
                        log.info("Associated with " + discovered.getOPEndpoint()
                                + " handle: " + assoc.getHandle());
                        break;
                    } else
                        log.info("Discarding association response, " +
                ...
                } else if (status == HttpStatus.SC_BAD_REQUEST) // error response
                {
                    ...
                    if (newReq != null) {
                        if (DEBUG) log.debug("Retrieved association type " +
    ...
    private AssociationRequest createAssociationRequest(
        ...
        try {
            ...
            if (assocReq == null)
                log.warn("Could not create association of type: " + type);
    ...
    public boolean verifyReturnTo(String receivingUrl, AuthSuccess response) {
        if (DEBUG)
            log.debug("Verifying return URL; receiving: " + receivingUrl +
        ...
        try {
            ...
            if (receivingParams == null) {
                if (DEBUG)
                    log.debug("Return URL query parameters verification failed.");
                return false;
            }
    ...
    public String insertConsumerNonce(String opUrl, String returnTo) {
        ...
        if (privateAssoc == null) {
            try {
                if (DEBUG) log.debug("Creating private association for opUrl " + opUrl);
                ...
                log.error("Cannot initialize private association.", e);
                return null;
            }
        }
        ...
        try {
            ...
            log.info("Inserted consumer nonce: " + nonce);

            if (DEBUG) log.debug("return_to:" + returnTo);
        } catch (Exception e) {
    ...
    public String extractConsumerNonce(String returnTo, String opUrl) {
        ...
        if (query == null) {
            log.error("Missing nonce in return_to query parameters, required for v1 responses");
            return null;
        }
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            try {
                if (keyVal.length == 2 && "openid.rpnonce".equals(keyVal[0])) {
                    ...
                    if (DEBUG) log.debug("Extracted consumer nonce: " + nonce);
                }

                if (keyVal.length == 2 && "openid.rpsig".equals(keyVal[0])) {
                    signature = URLDecoder.decode(keyVal[1], "UTF-8");
                    if (DEBUG) log.debug("Extracted consumer nonce signature: "
                            + signature);
                }
            } catch (UnsupportedEncodingException e) {
        ...
        if (DEBUG) log.debug("Consumer signed text:\n" + signed);

        try {
            if (DEBUG) log.debug("Loading private association for opUrl " + opUrl);
            Association privateAssoc = _privateAssociations.load(opUrl);
            if (privateAssoc == null) {
                log.error("Null private association.");
                return null;
            }
    ...
    private DiscoveryInformation verifyDiscovered1(AuthSuccess authResp,
        ...
        if (discovered != null && !discovered.isVersion2() &&
            ...
            // statefull mode
            if (DEBUG)
                log.debug("Verifying discovered information " +
        ...
        log.info("Proceeding with stateless mode / bare response verification...");
        ...
        while (iter.hasNext()) {
            ...
            if (DEBUG) log.debug("Found matching service: " + service);
            ...
            if (assoc != null) {
                if (DEBUG)
                    log.debug("Found existing association for  " + service +
    ...
    private VerificationResult verifySignature(AuthSuccess authResp,
        ...
        {
            log.info("Found association: " + assoc.getHandle() +
            ...
            if (assoc.verifySignature(text, signature)) {
                ...
                if (DEBUG) log.debug("Local signature verification succeeded.");
            } else {
        ...
        } else // no association, verify with the OP
        {
            ...
            if (HttpStatus.SC_OK == respCode) {
                ...
                if (DEBUG) log.debug("Error verifying signature with the OP: "
        ...
        if (verifiedID != null)
            ...
            log.error("Verification failed for: " + authResp.getClaimed()
