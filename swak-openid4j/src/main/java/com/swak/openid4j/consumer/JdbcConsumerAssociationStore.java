...
public class JdbcConsumerAssociationStore extends JdbcDaoSupport implements ConsumerAssociationStore {
    ...
    public Association load(String opUrl, String handle) {
        try {
            ...
            if (log.isDebugEnabled())
                log.debug("Retrieved association for handle: " + handle
                        + " from table: " + _tableName);

            return assoc;
        } catch (AssociationException ase) {
            log.error("Error retrieving association from table: "
                    + _tableName, ase);
            return null;
        } catch (IncorrectResultSizeDataAccessException rse) {
            log.warn("Association not found for handle: " + handle
                    + " in the table: " + _tableName);
            return null;
        } catch (DataAccessException dae) {
            log.error("Error retrieving association for handle: " + handle
    ...
    public Association load(String opUrl) {
        try {
            ...
            log.warn("Association not found for opUrl: " + opUrl
                    + " in the table: " + _tableName);
            return null;
        } catch (DataAccessException dae) {
            log.error("Error retrieving association for opUrl: " + opUrl
                    + "from table: " + _tableName, dae);
            return null;
        }
    }
    ...
    public void remove(String opUrl, String handle) {
        try {
            ...
            log.error("Error removing association from table: "
                            + _tableName,
                    e);
        }
    }
    ...
    public void save(String opUrl, Association association) {
        ...
        try {
            ...
            log.error("Error saving association to table: " + _tableName, e);
        }
    }
    ...
    private void cleanupExpired() {
        try {
            ...
            if (log.isDebugEnabled())
                log.debug("Client associations cleanup removed " + cnt
                        + " entries");
        } catch (Exception e) {
            log.error("Error cleaning up client associations from table: "
                    + _tableName, e);
        }
    }
