...
import com.swak.license.api.auth.*;
...
import static com.swak.license.api.io.bios.BIOS.*;
...
final class TrueLicenseManagementContext implements LicenseManagementContext, AuthenticationFactory, EncryptionFactory {
    ...
    TrueLicenseManagementContext(final TrueLicenseManagementContextBuilder b) {
        ...
        this.encryptionAlgorithm = GetterUtil.requireNonEmpty(b.encryptionAlgorithm);
        ...
        this.keystoreType = GetterUtil.requireNonEmpty(b.keystoreType);
        this.subject = GetterUtil.requireNonEmpty(b.subject);
            ...
            public License verify() throws LicenseManagementException {
                try {
                   return parent().verify();
                    ...
                    try {
                        return super.verify(); // uses store()
                    } catch (final LicenseManagementException second) {
                        synchronized (store()) {
                            try {
                                return  super.verify(); // repeat
                            } catch (final LicenseManagementException third) {
        ...
        class TrueLicenseManager
            ...
            public License install(final Source source) throws LicenseManagementException {
               return callChecked(() -> {
                    ...
                    License license = decodeLicense(source); // checks digital signature
                    copy(source, store());
                    return license;
                });
            }
            ...
            public License verify() throws LicenseManagementException {
               return callChecked(() -> {
                    ...
                    License license = decodeLicense(store());
    ...
    final class TrueLicenseInitialization implements LicenseInitialization {
        ...
        public void initialize(final License bean) {
            ...
            if (null == bean.getHolder()) {
                bean.setHolder(new X500Principal(CN_PREFIX + message(Messages.UNKNOWN)));
            }
